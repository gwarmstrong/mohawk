...
from mohawk.utils import full_fna_path
...
def simulate_from_genomes(id_list: List[str],
    ...
    for idx, (id_, depth) in enumerate(zip(id_list, id_depths)):
        if depth > 0:
            sequence_file = full_fna_path(sequence_directory, id_, lineage_info)
            sequences = list(skbio.io.read(sequence_file, format='fasta'))
            reads = sample_from_contig_set(sequences, depth, length, random_func)
            all_reads.append(reads)

    reads = np.vstack(all_reads)

    classes = [id_ for id_, depth in zip(id_list, id_depths) for _ in
               range(depth)]

    return reads, classes
